...
(function() {
  ...
  TrailerSwift.loadMap = function(lat, lng) {
    var mapOptions;
    ...
    return TrailerSwift.map = new google.maps.Map(document.getElementById("map_canvas"), mapOptions);
  };
...
(function() {
  ...
  TrailerSwift.Models.Location = Backbone.Model.extend({
    initialize: function(options) {
      console.log("initializing location " + (this.get('reverse')));
      return this.setLatLng();
    },
    setLatLng: function() {
      return this.set('latLng', TrailerSwift.Support.latLngFrom(this));
    },
    setMarker: function() {
      console.log('setting marker');
      return this.set('marker', TrailerSwift.Support.markerFrom(this));
    },
    showMarker: function() {
      if (this.has('marker') === false) {
        return this.setMarker();
      } else {
        return this.get('marker').setMap(TrailerSwift.map);
      }
    },
    hideMarker: function() {
      if (this.has('marker') === true) {
        return this.get('marker').setMap(null);
      }
    }
  });
...
(function() {
  ...
  TrailerSwift.Views.App = Backbone.View.extend({
    ...
    render: function() {
      ...
      return TrailerSwift.vanMarker = TrailerSwift.Support.imageMarker(TrailerSwift.locations.last(), vanImage);
    }
  });
...
(function() {
  ...
  $.extend(TrailerSwift.Support, {
    latLngFrom: function(location) {
      return new google.maps.LatLng(location.get('lat'), location.get('lng'));
    },
    markerFrom: function(location) {
      return new google.maps.Marker({
        position: this.latLngFrom(location),
        title: location.get('reverse'),
        map: TrailerSwift.map
      });
    },
    polyLineFromLocations: function() {
      var coords, lineSymbol;
      lineSymbol = {
        path: 'M 0,-1 0,1',
        strokeOpacity: 0.9,
        scale: 4
    ...
    imageMarker: function(location, imagePath) {
      return new google.maps.Marker({
